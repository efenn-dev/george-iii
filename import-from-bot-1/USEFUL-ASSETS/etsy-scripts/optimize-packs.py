...
Configuration:
    Edit ONEDRIVE and OUTPUT paths below for your system.
"""
import os
import zipfile
import warnings
...
# Example Linux/Mac:
# ONEDRIVE = Path("/home/user/OneDrive/Pictures")
# OUTPUT = Path("~/.openclaw/workspace/etsy-shop/ready-to-upload").expanduser()
#
# Example Windows:
ONEDRIVE = Path(r"C:\Users\efenn\OneDrive\Pictures")
OUTPUT = Path(r"C:\Users\efenn\.openclaw\workspace\etsy-shop\ready-to-upload")

MAX_ZIP_MB = 19.5  # Etsy limit is 20MB, stay under
MAX_PX = 2500      # Resize images to max 2500px on longest side
LICENSE = "Commercial use for printed physical products. Resale NOT permitted."


def optimize_png(src: Path, dst: Path, max_px: int) -> int:
    """Resize PNG to max_px on longest side, save optimized version."""
    img = Image.open(src)
    w, h = img.size
    if max(w, h) > max_px:
        r = max_px / max(w, h)
...
def process_pack(pack_dir: Path):
    """Process a single pack: resize, zip, split if needed."""
    name = pack_dir.name
    opt_dir = ONEDRIVE / f"{name}_optimized"
    opt_dir.mkdir(exist_ok=True)

    pngs = sorted(pack_dir.glob("*.png")) + sorted(pack_dir.glob("*.PNG"))
    if not pngs:
        print(f"  SKIP: no PNGs in {pack_dir}")
        return
    ...
    zp.unlink()
    print(f"  {mb:.1f}MB too big, splitting into parts...")
    license_file = opt_dir / "README-LICENSE.txt"

    # Group files to keep each part under limit
    groups = []
    current = []
    current_size = 0
    for f in opt_pngs:
...
def main():
    """Process all packs found in ONEDRIVE."""
    print("=== Etsy Pack Optimizer ===\n")
    OUTPUT.mkdir(exist_ok=True, parents=True)

    # Find all Pack* directories (excluding _optimized folders)
    packs = sorted([
